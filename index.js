...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    let index = Math.floor(
      Math.random(characters) * characters.length,
    );
    // console.log(Math.random(characters));
...
function calculateCircleArea(radius) {
  ...
  if (typeof radius !== 'number') {
    console.log('Помилка! Вхідні данні мають бути числом!');
    return null;
  }
...
function findMinMax(numbers) {
  ...
  if (!Array.isArray(numbers)) {
    console.log(
      'Помилка! Вхідні данні мають бути масивом чисел!',
    );
    return null;
  }
...
function calculateHypotenuse(a, b) {
  ...
  if (typeof a !== 'number' && typeof b !== 'number') {
    console.log(
      'Помилка! Вхідні данні мають бути числами!',
    );
    return null;
  }
  let hypot = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
  return `Довжина гіпотенузи для прямокутного трикутника з катетами довжиною ${a} та ${b} дорівнює: ${hypot}`;
}
...
function getFractionalPart(num) {
  ...
  //   const diff = num - integer;
  //   return Math.fround(diff);
...
console.log(
  isTotalPriceExceedsMaxPrice(products, maxPrice),
);
// Виведе: false
